% OP_F_SV: assemble the right-hand side vector r = [r(i)], with  
%           r(i) = (f, S v_i) and S is the Schrödinger operator.
%
...
function rhs = op_f_sv_st(spv,spt,msh,mshT,coeff)
...
shpv_lap  = reshape (spv.shape_function_laplacians, spv.ncomp, msh.nqn, spv.nsh_max, msh.nel);
shpt  = reshape (spt.shape_functions, spt.ncomp, mshT.nqn, spt.nsh_max, mshT.nel);
shpt_gra  = reshape (spt.shape_function_gradients, spt.ncomp, mshT.nqn, spt.nsh_max, mshT.nel);
...
for iel = 1:msh.nel
    if (all (msh.jacdet(:,iel)))
        ...
        shpv_lap_iel = repmat(reshape(shpv_lap(:,:,:,iel),spv.ncomp,msh.nqn,spv.nsh_max,1,1),[1,1,1,mshT.nqn,spt.nsh_max]);
        shpt_iel = repmat(reshape(shpt(:,:,:,ielT),spt.ncomp,1,1,mshT.nqn,spt.nsh_max),[spv.ncomp,msh.nqn,spv.nsh_max,1,1]);
        shpt_gra_iel = repmat(reshape(shpt_gra(:,:,:,ielT),spt.ncomp,1,1,mshT.nqn,spt.nsh_max),[spv.ncomp,msh.nqn,spv.nsh_max,1,1]);
        
        shpv_iel = reshape(permute((1i*shpv_iel.*shpt_gra_iel - shpv_lap_iel.*shpt_iel),[1 2 4 3 5]),spv.ncomp,msh.nqn*mshT.nqn,spv.nsh_max,spt.nsh_max);
       
        aux_val = bsxfun (@times, coeff_times_jw, conj(shpv_iel));
        ...
        warning ('geopdes:jacdet_zero_at_quad_node', 'op_f_sv: singular map in element number %d', iel)
    end
end
end
