...
% Source term
problem_data.f   = @(x, y, z, t) (4i*omg^2 + 4*x.^2 + 4*y.^2 + 4*z.^2 + 2*omg^2*t)/omg^4.*problem_data.uex(x, y, z, t);
problem_data.fx1 = {@(x) (4i*a0/omg^2)*esp(x), @(x) (4*a0*x.^2/omg^4).*esp(x), @(x) esp(x), @(x) esp(x), @(x) esp(x)};
problem_data.fx2 = {@(y) esp(y), @(y) esp(y), @(y) (4*a0*y.^2/omg^4).*esp(y), @(y) esp(y), @(y) esp(y)};
problem_data.fx3 = {@(z) esp(z), @(z) esp(z), @(z) esp(z), @(z) (4*a0*z.^2/omg^4).*esp(z), @(z) esp(z)};
problem_data.ft  = {@(t) esp(t), @(t) esp(t), @(t) esp(t), @(t) esp(t), @(t) (2*a0*t/omg^2).*esp(t)};
problem_data.h = @(x, y, z, t, ind) problem_data.uex(x, y, z, t);
problem_data.hx = @(x, y, z, ind) zeros (size (x)); % auxiliary function. 
problem_data.gmm = 1i;
problem_data.eta = 1;
problem_data.space_dimension = '3D'; 

% 2) CHOICE OF THE DISCRETIZATION PARAMETERS
clear method_data
n = 64;
d = 3;
method_data.trial_degree     = [d d d d-1]; % Degree of the splines (last is time dir)
method_data.trial_regularity = method_data.trial_degree-1; % Regularity of the splines
method_data.test_degree      = [d d d d-1]; % Degree of the splines (last is time dir)
method_data.test_regularity  = method_data.test_degree-1; % Regularity of the splines
method_data.nsub       = [n n n n]; % Number of subdivisions
...
%filename = ['smooth_3D/ST_SCHRODINGER_SMOOTH_2D_' method_data.preconditioner '_' method_data.solver '_degree_' num2str(d) '_Nt_' num2str(n*T) '_final_time_' num2str(T) '.mat'];
filename = ['smooth_3D_ps_pt/ST_SCHRODINGER_SMOOTH_3D_' method_data.preconditioner '_' method_data.solver '_ps_' num2str(d) '_pt_' num2str(d-1) '_Nt_' num2str(n*T) '_final_time_' num2str(T) '.mat'];
save(filename)
fprintf ('The result is saved in the file: %s \n \n', filename);
