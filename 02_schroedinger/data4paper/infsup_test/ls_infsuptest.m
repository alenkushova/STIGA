function [mu, D, geo, msh, space, eigvect] = ls_infsuptest (p, nel)
...
Mv = A + L2; % graph norm on trial space (splines).
Mw = A; % L^2-norm on test space A*(splines) --> so graph semi-norm.
